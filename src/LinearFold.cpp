...
using namespace std;
    ...
    bool compare_by_score(State&a, const State& b) {
...
value_type BeamCKYParser::beam_prune(int j, std::vector<State>& beamstep) {
    scores.clear();

    for (int i = 0; i <= j; ++i) {
        if (beamstep[i].score != VALUE_MIN) {
            int k = i - 1;
            value_type newscore;
            // lisiz: for _V, avoid -inf-int=+inf
            if ((k >= 0) && (bestC[k].score == VALUE_MIN)) newscore = VALUE_MIN;
            else newscore = (k >= 0 ? bestC[k].score : 0) + beamstep[i].score;
            scores.emplace_back(newscore, i);
        }
    }
    ...
    for (auto &p : scores) {
        if (p.first < threshold) beamstep[p.second] = State();
    }
...
void BeamCKYParser::prepare(unsigned len) {
    ...
    bestH = std::vector<std::vector<State>>(seq_length, std::vector<State>(seq_length));
    bestP.clear();
...
//             {
                ...
                sort(beamstepH.begin(), beamstepH.end(), compare_by_score);
#endif
